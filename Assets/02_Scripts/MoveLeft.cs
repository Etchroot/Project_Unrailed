...
public class MoveLeft : MonoBehaviour
{
    ...
    IEnumerator TrainLeft(Action action)
    {
        ...
        // Vector3 center = transform.position + (transform.Left * 3f);
        // Vector3 localTargetPoint = transform.position +
        //                    (transform.forward * 3f) +    // 로컬 기준 위 방향 (Y축)으로 3
        //                    (transform.Left * 3f); // 로컬 기준 오른쪽 방향 (X축)으로 3
        // Debug.Log($"{center}");
